    ...
    */
    ...
    public function ssoBPSRedirect(Request $request)
    {
        if (Auth::check()) {
            ...
            if (!$request->has('code')) {
                ...
                // Setelah mendapatkan token, ambil data email user untuk login ke aplikasi
                try {
                    ...
                    if ($kode_satker == '5207') {
                        $user = User::where('email', $data_sso->getEmail())->first();
                        if (!$user) {
                            $user = User::create([
                                'nip' => $data_sso->getNip(),
    ...
    public function bypassLogin(Request $request)
    {
        try {
            ...
            if (!$user) {
                exit('Gagal bypass: tidak ada email seperti itu terdaftar');
            }
            Auth::login($user);
            return redirect('/');
        } catch (\Exception $e) {
