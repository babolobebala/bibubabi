...
const handleDelete = () => {
    ...
    router.delete(destroy.url(props.drive.id), {
        onSuccess: () => {
...
<template>
    <Dialog :open="open" @update:open="(val) => !isSubmitting && emit('update:open', val)">
        <DialogContent class="sm:max-w-[425px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
                <DialogTitle class="text-destructive">Hapus Drive?</DialogTitle>
                <DialogDescription v-if="drive">
                    Apakah Anda yakin ingin menghapus drive <span class="font-semibold text-foreground">{{ drive.nama }}</span>?
                    <br />
            ...
            <DialogFooter class="mt-4">
                ...
                <Button 
                    ...
                    :disabled="isSubmitting"
                >
                    {{ isSubmitting ? 'Menghapus...' : 'Ya, Hapus Drive' }}
                </Button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
